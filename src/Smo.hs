...
{-# SPECIALIZE smoC :: Double -> Double -> Label -> Matrix Double -> (Double, UV.Vector Double) #-}
{-# SPECIALIZE smoC :: Double -> Double -> Label -> Matrix Float -> (Double, UV.Vector Double) #-}
smoC :: (UV.Unbox a,RealFloat a) =>
        Double ->    -- ^ Cp for y_i = 1
        Double ->    -- ^ Cn for y_i = -1
        Label ->     -- ^ y
        Matrix a ->  -- ^ Q[i][j] = y[i]*y[j]*K[i][j]; K: kernel matrix
        (Double, UV.Vector Double) -- ^ rho and alpha
smoC !costP !costN !y !mQ = let l = UV.length y
                                vAlpha = UV.replicate l 0.0
                                vGradient = UV.replicate l (-1.0)
                            in go vAlpha vGradient 0
  where
    cost y i = if y `atUV` i == 1
               then costP
               else costN
    !len = UV.length y
    eps=1e-3 :: Double
    ...
    {-# INLINE go #-}
    go :: UV.Vector Double -> UV.Vector Double -> Int -> (Double, UV.Vector Double)
